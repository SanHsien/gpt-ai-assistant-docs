...
- 行程的提前提醒預設改為**提前一天**（`REMINDER_OFFSETS=1440`），並保留原本的到點提醒；想只留到點提醒可把該值設為空字串。
- **有期限的任務**也會提醒了：與行程共用同一組提前量與到期提醒，並沿用安靜時段、暫停／恢復與過期跳過。沒有期限的任務不排提醒。
- 連結 Google 行事曆後，會**匯入 primary 日曆上既有的未來單次（非週期、有時刻）行程**，包含不是由 bot 建立的；之後在 Google 端新增／修改／刪除也會同步建立、重排或取消 LINE 提醒。全天、週期行程與非 primary 日曆仍不支援，且不需要重新授權更大的權限。
- 維護面：新增依賴自動檢查與每月新鮮度盤點，安全性更新與相容更新會自動處理，執行期依賴與重大版本仍由人工審查。
