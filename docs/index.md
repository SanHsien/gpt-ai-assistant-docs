...
目前文件對應獨立維護的 [SanHsien/gpt-ai-assistant](https://github.com/SanHsien/gpt-ai-assistant) **v6.1.0**。

本專案已上線 Supabase durable queue、文字與**語音**建行程、週期／多重提醒、Google Calendar／Tasks 同步、天氣與從搜尋建立行程。`6.0.0` 支援 LINE 桌面音訊附件、依 Content API 回應保留實際 MP3／WAV／M4A／WebM 格式，並容忍「記行程」常見同音辨識；集中 LINE／Supabase／Google 驗收已完成。`6.1.0` 進一步把行程提前提醒的預設改為**提前一天**（仍保留到點提醒）、讓**有期限任務**也有提前與到期提醒，並在 Google 行事曆連結後**匯入 primary 日曆上既有的未來單次行程**（含非 bot 建立者）一併提醒。
